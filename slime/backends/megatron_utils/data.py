...
def log_rollout_data(
    ...
    if mpu.get_tensor_model_parallel_rank() == 0 and mpu.is_pipeline_last_stage():
        ...
        # D3-MOPD: per-domain breakdown for opd_reverse_kl. Emits one key per
        # domain (not per fine-grained data_source) — the downstream filter /
        # watcher decisions are at domain level, and a domain-level emission
        # keeps wandb aligned with the N-teacher / N-domain training setup.
        # Skipped silently when either ``data_sources`` isn't surfaced or
        # ``D3MOPD_DOMAIN_MAP`` isn't set.
        #
        # ``D3MOPD_DOMAIN_MAP`` format (typically set by the launch script):
        #   "math=math_dapo,acereason_math|code=simulation__codeio,...|if=..."
        ds_list = rollout_data.get("data_sources")
        opd_per_sample = rollout_data.get("opd_reverse_kl")
        domain_map_env = os.environ.get("D3MOPD_DOMAIN_MAP", "")
        ds_to_domain: dict[str, str] = {}
        if domain_map_env:
            for entry in domain_map_env.split("|"):
                if "=" not in entry:
                    continue
        ...
        # Build per-rank domain -> sample-indices map. Empty if any of the
        # required ingredients (data_sources field, opd_reverse_kl, domain
        # map env) is missing — but participation in the all-gather below
        # is unconditional to keep the DP group in lockstep.
        domain_to_indices: dict[str, list[int]] = {}
        if (
            ds_list is not None
            and opd_per_sample is not None
            and len(ds_list) == len(opd_per_sample)
            and ds_to_domain
        ):
            ...
            for i, ds in enumerate(ds_list):
                if not ds:
                    continue
                domain = ds_to_domain.get(str(ds))
                if domain:
                    buckets[domain].append(i)
            domain_to_indices = buckets

        # Rare domains (with ~8 samples/rank from a 1:1:1 pool, ~4% chance a
        # rank lacks a given domain) can be absent on some DP ranks. The
        # ``for key in log_dict`` reduce on the src rank would KeyError if
        # any gathered rank missed the key, so every rank emits the same
        # union of domain keys, with (0.0, 0) placeholders where empty.
        #
        # We also all-gather per-domain LOCAL counts so we can compute each
        # domain's DP-total sample count. The emitted (sum, count) must have
        # count = DP-total-domain-samples, NOT num_rollouts_in_rollout — the
        # latter divides by all-domain totals and turns wandb per-domain KL
        # into (domain_ratio × true_mean), which becomes a positive-feedback
        # signal for dynamic ratio modes (dyn-gap / composite). Bug found
        # 2026-07-07 after M5d math independence pathology traced to this.
        gather_size = mpu.get_data_parallel_world_size(with_context_parallel=True)
        gather_group = mpu.get_data_parallel_group_gloo(with_context_parallel=True)
        local_domain_counts = {d: len(idx) for d, idx in domain_to_indices.items()}
        gathered_counts: list = [None] * gather_size
        dist.all_gather_object(gathered_counts, local_domain_counts, group=gather_group)
        # CP ranks within a DP group see identical samples, so each DP-rank's
        # dict shows up cp_size times in the gather. Divide out to recover
        # true DP-total per-domain sample count.
        per_domain_total_counts: dict[str, int] = {}
        for g in gathered_counts:
            if not g:
                continue
            for d, c in g.items():
                per_domain_total_counts[d] = per_domain_total_counts.get(d, 0) + c
        per_domain_total_counts = {d: c // cp_size for d, c in per_domain_total_counts.items()}
        union_domains = sorted(per_domain_total_counts.keys())

        for domain in union_domains:
            indices = domain_to_indices.get(domain, [])
            n_domain_dp_total = per_domain_total_counts.get(domain, 0)
            if n_domain_dp_total == 0:
                # No rank had any samples for this domain — degenerate.
                log_dict[f"opd_reverse_kl/{domain}"] = (0.0, 0)
                continue
            # Local sum contribution (0 if this rank has no domain-d samples).
            # Ranks-with-0-samples must still emit the per-rank COUNT share
            # (num_rollouts_in_rollout / dp_world) so Σcount lands on the
            # correct total. Emitting (0.0, 0) would under-count and inflate
            # the reported mean.
            if indices:
                d_max_seq_lens = (
                    [max_seq_lens[i] for i in indices] if max_seq_lens is not None else None
                )
                d_sum_of_sample_mean = get_sum_of_sample_mean(
                    [total_lengths[i] for i in indices],
                    [response_lengths[i] for i in indices],
                    [loss_masks[i] for i in indices],
                    sample_denoms=None,
                    qkv_format=args.qkv_format,
                    max_seq_lens=d_max_seq_lens,
                )
                d_tensor = torch.cat([opd_per_sample[i] for i in indices]).clone().detach()
                rank_sum = d_sum_of_sample_mean(d_tensor).item()
            else:
                rank_sum = 0.0
            # Emit (sum, count) so gather reduces to true per-sample mean
            # RESTRICTED to this domain — same shape as the global emit above
            # but with num_rollouts_in_rollout replaced by DP-total samples in
            # THIS domain. Post-gather: Σsum / Σcount = mean per-sample-mean
            # KL over domain d samples, independent of domain batch share.
            log_dict[f"opd_reverse_kl/{domain}"] = rollout_log_metric_contribution(
                rank_sum,
                cp_size=cp_size,
                num_rollouts_in_rollout=n_domain_dp_total,
                dp_size=dp_world,
            )
