...
class RolloutManager:
    ...
    def _convert_samples_to_train_data(self, samples: list[Sample] | list[list[Sample]]):
        ...
        # D3-MOPD: surface per-sample data_source so train side can bucket metrics
        # per domain. No-op for runs whose samples lack metadata.data_source.
        data_sources = [
            (sample.metadata or {}).get("data_source") if sample.metadata else None for sample in samples
        ]
        if any(ds is not None for ds in data_sources):
            train_data["data_sources"] = data_sources
